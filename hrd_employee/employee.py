...
class employee(osv.osv):
    ...
    def name_get(self):
        ...
        for asset in self:
            display_name = []
            res.append((asset.id,
                        '['+ asset.name + ']' + ' ' + asset.employee_name))
        return res
    ...
    _columns = {
        'name_related': fields.related('resource_id', 'name', type='char', string='NIK', store=True),
        'employee_name' : fields.char('Name', required=True),
        'nik': fields.char('NIK Tidak Aktif',20),
        'nik1': fields.char('NIK Tidak Aktif 1',20),
        'nik2': fields.char('NIK Tidak Aktif 2',20),
        'nik3': fields.char('NIK Tidak Aktif 3',20),
        'nik4': fields.char('NIK Tidak Aktif 4',20),
        'nik5': fields.char('NIK Tidak Aktif 5',20),
        'nik6': fields.char('NIK Tidak Aktif 6',20),
        'nik7': fields.char('NIK Tidak Aktif 7',20),
        'nik8': fields.char('NIK Tidak Aktif 8',20),
        'nik9': fields.char('NIK Tidak Aktif 9',20),
        'nik10': fields.char('NIK Tidak Aktif 10',20),
        'nik11': fields.char('NIK Tidak Aktif 10',20),
        'nik12': fields.char('NIK Tidak Aktif 10',20),
        'nik13': fields.char('NIK Tidak Aktif 10',20),
        'nik14': fields.char('NIK Tidak Aktif 10',20),
        'nik15': fields.char('NIK Tidak Aktif 10',20),
        'nik16': fields.char('NIK Tidak Aktif 10',20),
        'nik17': fields.char('NIK Tidak Aktif 10',20),
        'nik18': fields.char('NIK Tidak Aktif 10',20),
        'nik19': fields.char('NIK Tidak Aktif 10',20),
        'nik20': fields.char('NIK Tidak Aktif 10',20),
        'kelamin': fields.selection([('L','Laki-laki'),('W','Perempuan')],'Jenis Kelamin',required=False),
        ...
        'marital' : fields.selection([('menikah', 'Menikah'), ('belum_menikah', 'Belum Menikah')], 'Marital Status'),
        }

    _defaults = {    
        'name': lambda self, cr, uid, context={}: self.pool.get('ir.sequence').get(cr, uid, 'hr.employee.nik'),
        'jenis_id': 'KTP',
        'tgl_masuk' : lambda *a: time.strftime('%Y-%m-%d'),
                }
    # def _compute_mesin_count(self):
    #     # read_group as sudo, since contract count is displayed on form view
    #     mesin_data = self.env['no.mesin'].sudo().read_group([('employee_id', 'in', self.ids)], ['employee_id'], ['employee_id'])
    #     result = dict((data['employee_id'][0], data['employee_id_count']) for data in mesin_data)
    #     import pdb;pdb.set_trace()
    #     for employee in self:
    #         employee.mesin_count = result.get(employee.id, 0)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        src = obj.search(cr,uid,[('name','=',nik)])
        for emp in obj.browse(cr,uid,src):
            raise osv.except_osv(_('Warning!'),_('NIK Tidak Boleh Sama'))
        return super(employee,self).create(cr,uid,vals,context=context)
employee()
...
class divisi(osv.osv):
    ...
    _columns = {
        ...
        'department_id' : fields.many2one('hr.department','departmen'),
...
class resource_resource(osv.osv):
    ...
    _columns = {
        'name' : fields.char("NIK", required=True)
    }
resource_resource()

class no_mesin(osv.osv):
    _name = "no.mesin"

    _columns = {
        'employee_id' : fields.many2one('hr.employee', 'NIK'),
        'date' :fields.date('Sejak Tanggal'),
        'no_mesin' : fields.integer('No Mesin'),
        'fingerprint_code' : fields.char('fingerprint Code')
    }